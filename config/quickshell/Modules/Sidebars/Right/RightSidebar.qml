...
PanelWindow {
    ...
    // Clamp the dynamic panel height to a sensible range. The QuickSettings
    // container reports its active view's preferred content height; for
    // compact views (e.g. power menu at ~180px) this shrinks the panel,
    // for the others it stays at the full 640px.
    readonly property int qsPanelHeight: {
        const requested = quickSettingsLoader.item
            ? quickSettingsLoader.item.contentImplicitHeight
            : 640;
        return Math.max(180, Math.min(640, requested));
    }
    ...
    Item {
        ...
        height: root.inputActive ? root.qsPanelHeight : 0
    }
