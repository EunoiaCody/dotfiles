...
class Corner {
  ...
  calculateDirectionAlignment(cursorDimensions, destination) {
    ...
    return (
      travelDirection.x * cornerDirection.x +
      travelDirection.y * cornerDirection.y
    );
  }
  ...
  jump(destination, cursorDimensions, rank) {
    ...
    const isShortJump =
      Math.abs(jumpVec.x) <= 2.001 && Math.abs(jumpVec.y) <= 0.001;

    if (isShortJump) {
      this.animationLength = Math.min(
        ANIMATION_SETTINGS.animationLength,
        ANIMATION_SETTINGS.shortAnimationLength
      );
    } else {
      const leading =
        ANIMATION_SETTINGS.animationLength *
        clamp(1 - ANIMATION_SETTINGS.trailSize, 0, 1);
  ...
  update(cursorDimensions, destination, dt, immediate) {
    const cornerDestination = this.getDestination(
      destination,
      cursorDimensions
    );

    if (
      cornerDestination.x !== this.previousDestination.x ||
      cornerDestination.y !== this.previousDestination.y
    ) {
      const delta = {
        x: cornerDestination.x - this.currentPosition.x,
        y: cornerDestination.y - this.currentPosition.y,
...
function createNeovideCursor(options) {
  ...
  function move(x, y) {
    ...
    if (!initialized) {
      corners.forEach((corner) => {
        const cornerDest = corner.getDestination(
          centerDestination,
          cursorDimensions
        );
  ...
  function drawCursorShape() {
    ...
    for (let i = 1; i < corners.length; i++) {
      context.lineTo(
        corners[i].currentPosition.x,
        corners[i].currentPosition.y
      );
    }
    context.closePath();
  ...
  function updateLoopLogic(isScrolling, shouldDraw) {
    ...
    if (jumped) {
      const ranks = computeCornerRanks(
        corners,
        cursorDimensions,
        centerDestination
      );
